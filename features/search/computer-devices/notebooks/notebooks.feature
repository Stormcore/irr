...
Функционал: Компьютерная техника -> Ноутбуки -> Ноутбуки
  ...
  Сценарий: Фильтр по типу видеокарты
   Когда я делаю поиск по следующим параметрам:
      | parameter      | value     |
      | Тип видеокарты | втроенная |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях каждого объявления "Тип видеокарты" равно "втроенная"
  ...
  Сценарий: Фильтр по времени работы от батареи
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях каждого объявления "Время работы от батареи" >= "2"
       И в деталях каждого объявления "Время работы от батареи" <= "10"
  ...
  Сценарий: Фильтр по беспроводным интерфейсам
   Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях каждого объявления "Беспроводные интерфейсы" равно "Wi-Fi"
