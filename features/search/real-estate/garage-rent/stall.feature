# language:ru
Функционал: Недвижимость -> Гаражи и стоянки. Аренда -> Машиноместо

  Контекст:
    Когда открыта страница для города "Москва"
        И на главной странице я перехожу в категорию "Недвижимость -> Гаражи и стоянки. Аренда -> Машиноместо"
  ...
  Сценарий: Фильтр по типу стоянки
    Когда я делаю поиск по следующим параметрам:
      | parameter   | value    |
      | Тип стоянки | закрытая |
      И на странице поиска загружен список результатов для недвижимости

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип стоянки" равно "закрытая"
  ...
  Сценарий: Фильтр по цене
    Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       ...
       И в каждом объявлении срок сдачи равена "руб."
  ...
  Сценарий: Фильтр по территории
    Когда я делаю поиск по следующим параметрам:
      | parameter                | value |
      | На территории ГСК или ГК | x     |
      И на странице поиска загружен список результатов для недвижимости

      То на странице показано >= 1 объявлений
       И в деталях первого объявления присутствует "На территории ГСК или ГК"
