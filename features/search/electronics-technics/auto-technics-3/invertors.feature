...
Функционал: Электроника и техника -> Техника для авто -> Инверторы
  ...
  Сценарий: Фильтр по выходному напряжению
    Когда я делаю поиск по следующим параметрам:
      ...
      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Выходное напряжение" >= "1"
       И в деталях первого объявления "Выходное напряжение" <= "1000"

  Сценарий: Фильтр по выходной мощности
    Когда я делаю поиск по следующим параметрам:
        | parameter         | min | max  |
        | Выходная мощность | 1   | 1000 |
       И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Выходная мощность" >= "1"
       И в деталях первого объявления "Выходная мощность" <= "1000"

  Сценарий: Фильтр по пиковая мощности
    Когда я делаю поиск по следующим параметрам:
        | parameter        | min | max  |
        | Пиковая мощность | 1   | 1000 |
       И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Пиковая мощность" >= "1"
       И в деталях первого объявления "Пиковая мощность" <= "1000"
