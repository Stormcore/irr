# language:ru
# encoding: utf-8
Функционал: Оборудование -> Строительное и ремонтное

  Контекст:
    Когда открыта страница для региона "Казахстан"
        И на главной странице я перехожу в категорию "Оборудование -> Строительное и ремонтное"
...
@customfield
  Сценарий: Фильтр по типу оборудования
   Когда я делаю поиск по следующим параметрам:
      | parameter        | value                 |
      | Тип оборудования | воздушные компрессоры |
      И на странице поиска загружен список результатов

      То на странице показано >= 1 объявлений
       И в деталях первого объявления "Тип оборудования" равно "воздушные компрессоры"
