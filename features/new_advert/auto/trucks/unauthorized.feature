  ...
  Сценарий: Подача объявления неавторизированным пользователем
    Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
       ...
       * в ЛК ИП я запоминаю количество счетчика для категории "Авто и мото -> Коммерческий транспорт -> Средние и тяжелые грузовики"
        * я выхожу из текущего пользователя
        ...
        * я ввожу следующие данные на шаге 2:
          ...
          | Марка                | DAF            |
          | Модель               | F1700          |
          | Заголовок:           | Продаю неавторизированным пользователем |
          | Цена                | 10000          |
          | Валюта               | $              |
          | Объем двигателя:     | 10             |
          | Пробег:              | 10000          |
...
@advert_submit
  Сценарий: Проверка деталей поданного объявления
    Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
       ...
       То в ЛК ИП объявление с названием "Продаю неавторизированным пользователем" присутствует в списке
...
@after_index
  Сценарий: Проверка индексации
   Когда на главной странице я перехожу в категорию "Авто и мото -> Коммерческий транспорт -> Средние и тяжелые грузовики"
       ...
       * в списке обычных объявлений присутствует объявление "Продаю неавторизированным пользователем"
