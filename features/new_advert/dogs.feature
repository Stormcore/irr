...
@after_index
  Сценарий: Проверка создания объявления
   Когда открыта страница для региона "Усть-Алтан"
       ...
       * в списке премиумов присутствует объявление "Тест подачи объявления"
...
@new_advert
   ...
   * у объявления указан регион "Москва"
...
@after_index
  Сценарий: Проверка редактирования объявления
   Когда открыта страница для региона "Усть-Алтан"
       * на главной странице я перехожу в категорию "Животные и растения -> Животные -> Собаки"
       * на странице поиска загружен список результатов
       * в списке премиумов присутствует объявление "Тест редактирования - новый заголовок"
...
@new_advert
  Сценарий: Создание премиума через БО
   Когда я перехожу на страницу stargate
      ...
      | Цена                | 100                       |
      | Валюта              | $                         |
      | Владелец объявления | kruglova@yandex.ru        |
      | Порода              | Английский кокер-спаниель |
      | Пол                 | мальчик                   |
      | Возраст             | 5                         |
      | age_name            | мес.                      |
