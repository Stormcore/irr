...
@new_advert
  ...
  То объявление с названием "Отдаю щенков спаниэля" присутствует в списке
   ...
   * у объявления указана цена "740" в евро
   * у объявления указана цена "29030" в рублях
...
@after_index
  Сценарий: Проверка поднятия
   Когда открыта страница для региона "Усть-Алтан"
       ...
       * первым в списке обычных объявлений первым идёт объявление "Отдаю щенков"
...
@after_index
  Сценарий: Проверка выделения
   Когда открыта страница для региона "Усть-Алтан"
       * на главной странице я перехожу в категорию "Животные и растения -> Животные -> Собаки"
       * на странице поиска загружен список результатов
       * в списке обычных объявлений объявление "Продам красивых щенков" выделено
...
@new_advert
  Сценарий: Смена статуса на премиум
   Когда я вхожу под пользователем с ролью "Интернет-партнер"
       ...
       * в ЛК ИП я выбираю пакет "NEW Животные и растения | ВСЕ РЕГИОНЫ"
       * в ЛК ИП я запоминаю значение поля "Выделение"
       * объявление с названием "Продам красивых щенков" присутствует в списке
       * я делаю данное объявление премиумом
      То в ЛК ИП значение поля "Премиум (всего)" уменьшилось на единицу
...
@after_index
  Сценарий: Проверка смена статуса на премиум
   Когда открыта страница для региона "Усть-Алтан"
       * на главной странице я перехожу в категорию "Животные и растения -> Животные -> Собаки"
       * на странице поиска загружен список результатов
       * в списке премиумов присутствует объявление "Продам красивых щенков"
