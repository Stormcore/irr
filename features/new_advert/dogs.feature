...
@new_advert
  Сценарий: Создание премиума через БО
   Когда я перехожу на страницу stargate
      ...
      #| Регион           | Усть-Алтан                | region |
      | Заголовок        | Тест создания премиума    | text   |
      | Текст объявления | Отдам щенков              | text   |
      | Тип предложения  | даром                     | select |
      | Цена             | 100                       | text   |
      | Валюта           | $                         | select |
      | Порода           | Английский кокер-спаниель | select |
      | Пол              | мальчик                   | select |
      | Возраст          | 5                         | text   |
      | age_name         | мес.                      | select |
