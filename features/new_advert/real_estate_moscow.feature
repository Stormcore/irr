...
@new_advert
  Сценарий: Смена статуса на премиум
   Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
     ...
     * открыт список объявлений пользователя
     * в ЛК ИП я выбираю пакет "NEW [TEST] Недвижимость"
     * объявление с названием "5-комн.  квартира,   Интернациональная ул,   10,  5/8,  площадь:  общая 500 кв. м.,  приватизированная" присутствует в списке
...
@after_index
  Сценарий: Проверка  смена статуса на премиум
   Когда открыта страница для региона "Москва"
       * на главной странице я перехожу в категорию "Недвижимость -> Квартиры. Продажа -> Вторичный рынок"
       * на странице поиска загружен список результатов
       * в списке премиумов присутствует объявление "5-комн.  квартира,   Интернациональная ул,   10,  5/8,  площадь:  общая 500 кв. м.,  приватизированная"