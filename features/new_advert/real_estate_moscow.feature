...
@new_advert
  Сценарий: Подача объявления с фотографией
   Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
       ...
       * я перехожу на шаг 3
       * открыт список объявлений пользователя
      То объявление с названием "5-комн.  квартира,   Интернациональная ул,   10,  5/8,  площадь:  общая 500 кв. м.,  приватизированная" присутствует в списке
       * у объявления указан город "Москва"
...
@after_index
  Сценарий: Проверка подачи объявления с фото
   Когда открыта страница для региона "Москва"
       ...
       * у объявления "5-комн.  квартира,   Интернациональная ул,   10,  5/8,  площадь:  общая 500 кв. м.,  приватизированная" отображается загруженная фотография
...
@new_advert
  Сценарий: Подача объявления с видео
   Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
       ...
       * у объявления указан город "Москва"
       * у объявления указана цена "500000" в долларах
       * у объявления отображается загруженное видео
...
@new_advert
  Сценарий: Поднятие объявления
   Когда я вхожу под пользователем с ролью "Доверенный интернет-партнер"
     ...
     * открыт список объявлений пользователя
     * в ЛК ИП я выбираю пакет "NEW [TEST] Недвижимость"
     * объявление с названием "7-комн.  квартира,   Интернациональная ул,   10,  7/8,  площадь:  общая 700 кв. м.,  приватизированная" присутствует в списке
...
@after_index
  Сценарий: Проверка поднятия
   Когда открыта страница для региона "Москва"
       * на главной странице я перехожу в категорию "Недвижимость -> Квартиры. Продажа -> Вторичный рынок"
       * на странице поиска загружен список результатов
       * в списке обычных объявлений присутствует объявление "7-комн.  квартира,   Интернациональная ул,   10,  7/8,  площадь:  общая 700 кв. м.,  приватизированная"
       * первым в списке обычных объявлений первым идёт объявление "7-комн.  квартира,   Интернациональная ул,   10,  7/8,  площадь:  общая 700 кв. м.,  приватизированная"