...
class AdDetailsPage
  ...
  def get_title
    self.title_element.when_present.text
  end

  def get_price currency
    self.price_element.element.spans.find {|s|
         s.html.gsub("&nbsp;",'').gsub(/<(\/|)span>/,'').include?(" #{currency}")
    }.html.gsub("&nbsp;",'').gsub(/<(\/|)span>/,'').gsub(" #{currency}", '')
  end

  def get_description
    self.description_element.paragraph_element.text
  end
